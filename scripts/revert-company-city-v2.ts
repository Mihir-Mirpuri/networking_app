...
async function revertAll() {
  // Bain people who "left" - need to revert or delete duplicate
  const bainLeft = [
  ...
  for (const item of bainLeft) {
    // Find the record at the new company
    const personAtNewCo = await prisma.person.findFirst({
      where: {
    ...
    if (!personAtNewCo) {
      console.log(`NOT FOUND at new company: ${item.name} @ ${item.currentCompany}`);
      continue;
    }

    // Check if original Bain record still exists
    const personAtBain = await prisma.person.findFirst({
      where: {
        fullName: { contains: item.name },
        company: { contains: 'Bain' },
      },
      select: { id: true, fullName: true, company: true }
    });

    if (personAtBain) {
      // Original exists, delete the new company record
      await prisma.person.delete({ where: { id: personAtNewCo.id } });
      console.log(`✓ ${item.name}: deleted duplicate at ${personAtNewCo.company} (kept Bain record)`);
      deleted++;
    } else {
      // Original doesn't exist, revert company name
      await prisma.person.update({
        where: { id: personAtNewCo.id },
        data: { company: 'Bain & Company' },
      });
      console.log(`✓ ${item.name}: ${personAtNewCo.company} → Bain & Company`);
      companyReverted++;
    }
  }
  ...
  console.log(`Reverted city/state to null for ${cityReverted} records`);
